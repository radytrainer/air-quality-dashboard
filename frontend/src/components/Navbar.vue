  ...
  <nav
    ...
    <div class="flex items-center gap-8">
      ...
      <div class="w-16 h-16 rounded-2xl ">
        <img src="../assets/images/Logo.png" alt="Air Quality Icon" class="w-full h-full object-cover" />
      </div>
      <!-- Search Bar -->
    ...
    <div class="hidden md:flex items-center gap-2 text-sm font-medium">
      <RouterLink to="/home"
        ...
        :class="$route.path === '/home' ? 'text-blue-600 bg-blue-50' : 'text-gray-700'">
        {{ $t('nav.home') }}
        <div v-if="$route.path === '/home'"
          class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-6 h-0.5 bg-blue-600 rounded-full"></div>
      </RouterLink>
      <RouterLink to="/city-detail"
        class="px-4 py-2 rounded-lg transition-all duration-300 hover:bg-blue-50 hover:text-blue-600 relative group whitespace-nowrap"
        :class="$route.path === '/city-detail' ? 'text-blue-600 bg-blue-50' : 'text-gray-700'">
        {{ $t('nav.cityDetail') }}
        <div v-if="$route.path === '/city-detail'"
          class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-6 h-0.5 bg-blue-600 rounded-full"></div>
      </RouterLink>
      <RouterLink to="/compare-cities"
        class="px-4 py-2 rounded-lg transition-all duration-300 hover:bg-blue-50 hover:text-blue-600 relative group whitespace-nowrap"
        :class="$route.path === '/compare-cities' ? 'text-blue-600 bg-blue-50' : 'text-gray-700'">
        {{ $t('nav.compareCities') }}
        <div v-if="$route.path === '/compare-cities'"
          class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-6 h-0.5 bg-blue-600 rounded-full"></div>
      </RouterLink>
    ...
    <div class="flex items-center gap-2 relative">
      ...
      <button
        ...
        <img src="https://flagcdn.com/us.svg" alt="US Flag" class="h-4 w-5 rounded-sm shadow-sm flex-shrink-0" />
    ...
    <!-- Mobile Menu Button -->
    <button class="md:hidden p-2 rounded-lg hover:bg-gray-100 transition-colors duration-200" @click="toggleMobileMenu">
    ...
    <div v-if="mobileMenuOpen"
      ...
      <div class="p-4 space-y-2">
        <RouterLink to="/home" class="block px-4 py-2 rounded-lg hover:bg-blue-50 hover:text-blue-600"
          @click="mobileMenuOpen = false">
          {{ $t('nav.home') }}
        </RouterLink>
        <RouterLink to="/city-detail" class="block px-4 py-2 rounded-lg hover:bg-blue-50 hover:text-blue-600"
          @click="mobileMenuOpen = false">
          {{ $t('nav.cityDetail') }}
        </RouterLink>
        <RouterLink to="/compare-cities" class="block px-4 py-2 rounded-lg hover:bg-blue-50 hover:text-blue-600"
          @click="mobileMenuOpen = false">
          {{ $t('nav.compareCities') }}
        </RouterLink>
        <RouterLink to="/analytics" class="block px-4 py-2 rounded-lg hover:bg-blue-50 hover:text-blue-600"
          @click="mobileMenuOpen = false">
          {{ $t('nav.analytics') }}
        </RouterLink>
      </div>
