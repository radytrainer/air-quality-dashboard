<template>
  <div class="min-h-screen flex relative overflow-hidden">
    ...
    <div class="flex-1 flex items-center justify-center px-8 relative z-20">
      <div class="w-full max-w-md">
        ...
        <div class="flex mb-8 border-b border-white/20">
          <button @click="$router.push('/login')" class="px-6 py-2 text-gray-300 hover:text-white">
            {{ $t('auth.login') }}
          </button>
          <button class="px-6 py-2 text-white border-b-2 border-white font-medium">
            {{ $t('auth.register') }}
          </button>
        </div>
        ...
        <form @submit.prevent="register" class="space-y-6">
          <!-- Full Name -->
          <div>
            <label class="block text-white text-sm font-medium mb-3">{{ $t('auth.name') }}</label>
            <input
              v-model="name"
              type="text"
          ...
          <div class="flex items-start mt-8">
            <input
              type="checkbox"
              v-model="acceptTerms"
              class="w-4 h-4 text-white bg-transparent border border-white/30 rounded focus:ring-white focus:ring-1 mt-1 accent-white"
            />
            <span class="ml-3 text-gray-300 text-sm">
              {{ $t('auth.termsText') }}
              <a href="#" class="text-white hover:underline">{{ $t('auth.terms') }}</a>
              {{ $t('auth.and') }}
              <a href="#" class="text-white hover:underline">{{ $t('auth.privacy') }}</a>
            </span>
          </div>
        ...
        <p class="text-center text-gray-300 mt-8">
          ...
          <button @click="$router.push('/login')" class="text-white hover:underline font-medium ml-1">
            {{ $t('auth.login') }}
          </button>
        </p>
    ...
    <div class="flex-1 flex flex-col justify-center px-16 relative z-20">
      <div class="max-w-md">
        <h1 class="text-5xl font-bold text-white mb-4">{{ $t('auth.registerTitle') }}</h1>
        <p class="text-xl text-gray-300">{{ $t('auth.registerSubtitle') }}</p>
      </div>
    </div>
  </div>
</template>
