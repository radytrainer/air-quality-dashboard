  ...
  <div class="min-h-screen bg-gray-50 p-6" v-click-outside="closeDropdown">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex flex-wrap gap-4 mb-6 bg-white p-4 rounded shadow">
        <input v-model="searchQuery" type="search" placeholder="Search name or email..."
      ...
      <div class="bg-white rounded shadow overflow-x-auto">
            ...
            <tr v-for="user in filteredUsers" :key="user.id">
              ...
              <td class="px-6 py-4 text-right">
                <button @click="toggleDropdown(user.id)"
                  class="dropdown-button text-gray-600 hover:text-gray-900 focus:outline-none" aria-haspopup="true"
                ...
                <div v-if="dropdownOpenId === user.id"
                  ...
                  " class="block w-full text-left px-4 py-2 text-sm text-gray-600 hover:bg-gray-100">
                    <svg class="w-4 h-4 inline-block mr-2" fill="none" stroke="currentColor" stroke-width="2"
                      viewBox="0 0 24 24">
        ...
        <div v-if="!loading && filteredUsers.length === 0" class="p-4 text-center text-gray-600">
          No users found
        </div>
      </div>
    </div>
    ...
    <div v-if="showAddUserModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded p-6 w-full max-w-md relative">
        ...
        <form @submit.prevent="submitAddUser" class="space-y-4 text-sm">
          <input v-model="addUserForm.name" type="text" placeholder="Name" class="w-full border rounded px-3 py-2"
            required />
          <input v-model="addUserForm.email" type="email" placeholder="Email" class="w-full border rounded px-3 py-2"
            required />
          <input v-model="addUserForm.password" type="password" placeholder="Password"
            class="w-full border rounded px-3 py-2" required />
          <input v-model="addUserForm.password_confirmation" type="password" placeholder="Confirm Password"
            class="w-full border rounded px-3 py-2" required />
          ...
          <div>
            ...
            <img v-if="addUserImagePreview" :src="addUserImagePreview" alt="preview"
              class="mt-2 w-24 h-24 rounded object-cover" />
          </div>
          <div class="flex justify-end gap-2 mt-4">
            <button type="button" @click="closeAddUser" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">
              Cancel
            </button>
      ...
      <div class="bg-white rounded p-6 w-full max-w-md relative">
        ...
        <form @submit.prevent="submitEditUser">
          ...
          <div class="mb-3">
            ...
            <input v-model="editUserForm.name" type="text" class="w-full border rounded px-3 py-2" required />
          </div>

          <!-- Email -->
          <div class="mb-3">
            <label class="block font-semibold">Email</label>
            <input v-model="editUserForm.email" type="email" class="w-full border rounded px-3 py-2" required />
          </div>

          <!-- Role -->
          <div class="mb-3">
            <label class="block font-semibold">Role</label>
            <select v-model="editUserForm.role" class="w-full border rounded px-3 py-2">
              <option value="user">User</option>
              <option value="admin">Admin</option>
              <option value="viewer">Viewer</option>
            </select>
          ...
          <div class="mb-3">
            ...
            <input v-model="editUserForm.phone" type="text" class="w-full border rounded px-3 py-2" />
          </div>
    ...
    <div v-if="showViewUserModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded p-6 w-full max-w-md relative">
        ...
        <div class="flex gap-4 mb-4 items-center">
          <img v-if="viewUserData.profile_image" :src="getFullImageUrl(viewUserData.profile_image)" alt="profile"
            class="w-20 h-20 rounded-full object-cover" />
          <div class="w-20 h-20 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-3xl"
            v-else>
